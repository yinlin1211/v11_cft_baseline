...
当前推荐汇报的是完整两阶段 `val40` 搜索结果：

1. `val40` 搜 `onset/frame`
2. 固定第一阶段结果，再在 `val40` 上搜 `offset`
3. 只在 `test100` 上评测一次
...
| Split | onset | frame | offset | COn | COnP | COnPOff |
|-------|------:|------:|-------:|----:|-----:|--------:|
| test100 | 0.45 | 0.50 | 0.10 | 0.803172 | 0.776425 | 0.592620 |

这就是当前仓库里最完整、最推荐对外使用的一组结果。
