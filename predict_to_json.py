...
用法：
  ...
  python predict_to_json.py \
      --config config.yaml \
      --checkpoint run/20260422_201016_COnP/checkpoints/best_model.pt \
      --split test \
      --onset_thresh 0.50 \
      --frame_thresh 0.40 \
      --offset_thresh 0.30 \
...
def frames_to_notes(frame_pred, onset_pred, hop_length, sample_rate,
    ...
    for p in range(P):
        ...
        if len(onset_frames) == 0:
            ...
            # 有 onset：offset 峰值优先，frame 连续区间兜底
...
def predict_from_npy(model, npy_path, config, device):
    """从 npy CQT 文件推理，返回 (frame_pred, onset_pred, offset_pred)，shape=(T, 48)"""
